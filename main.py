...
class BasicModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.transformer = nn.Transformer(num_encoder_layers=6, num_decoder_layers=6, batch_first=True, d_model=128)
        self.pe = positionalencoding1d(128, 1000).cuda()
        self.to_dist = nn.Sequential(
            nn.Linear(128, MathToken.size())
        )
    ...
    def forward(self, data, output):
        ...
        embedded_tgt = self.embed(tgt_mask * output) + self.pe[:output.shape[1]].unsqueeze(0)
        attn_mask = self.transformer.generate_square_subsequent_mask(output.shape[1]).cuda()
        out = self.transformer(embedded_data, embedded_tgt, src_key_padding_mask=~mask, tgt_key_padding_mask=~tgt_mask, tgt_mask=attn_mask)
